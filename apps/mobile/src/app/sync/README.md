...
- `startSync()` **resolves with the first sync cycle.** The boot deliberately does not wait for it (local-first: the hydrated store renders at once); `ensureIdentity()` does. The first share speaks to the server about a list that was written while the device was still purely local, so that list's `listCreated` has to leave the outbox first — otherwise the invite request arrives before the list and comes back 403.
- `pushQueuedEvents()` is the **barrier in front of such a command**: it waits for cycles until the send queue is empty and gives up when a cycle neither moved anything nor left a follow-up behind (offline — the caller gets an answer instead of an endless skeleton). `requestSync()` is no barrier: mid-cycle it hands back the *running* cycle, whose drain may already be past the new entry. It never starts a cycle while one runs, which would keep re-arming the coalesced follow-up and spin. It also covers the device that already has an account, where `startSync()` has nothing left to await.
