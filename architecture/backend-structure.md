...
- **Use Cases nehmen `&Ports`** — ein Bündel aus drei `&dyn`-Referenzen (`events`, `membership`, `broadcast`), einmal pro Cold Start verdrahtet. Verdrahtung und Request-Daten bleiben in der Signatur sichtbar getrennt (`append_event(&ports, &caller_id, &aggregate, request)`). Bewusst `dyn` statt Generics: `#[async_trait]` boxt ohnehin jede Future, der statische Dispatch hätte nur Typparameter-Lärm gekauft.
- **Namen sagen, was sie halten (2026-08-03):** Wer eine Id hält, heißt `…_id` — `caller_id`, `member_id`, `owner_id`, `list_id`, Sammlungen `…_ids`. Werte heißen nach ihrem Typ (`stored_event`, `validated_envelope`), nicht nach einem Adjektiv.
- **`aggregate` ist das Paar, `aggregate_id` der String (2026-08-03):** `Aggregate { kind, id }` ist keine Id, sondern das Handle auf einen Event-Log — genauso heißt es beim Client (`app/sync/aggregate.ts`) und auf dem Wire. Eine nackte Id heißt deshalb `aggregate_id` bzw. `list_id`/`recipeId`, niemals beides dasselbe. Der frühere Name `AggregateId` erzwang `aggregate_id.id` und machte zwei verschiedene Dinge namensgleich.
- **Ausnahme Port-Methoden:** die benennen die Beziehung, nicht den Rückgabetyp (`members_of`, `owner_of`, `aggregates_of`) — der Typ steht in der Signatur daneben. Ebenfalls unangetastet: Wire-Keys und persistierte Attribute (`"aggregate"`), die Client und DynamoDB lesen.
