...
async fn handle(
    ...
    match my_lists_with_owners(ports, users, &caller).await {
        Ok(summaries) => {
            // `lists` keeps its shape — the sync engine's catch-up reads it
            // as a plain id array. `ownerNames` is additive, so older
            // clients ignore it and nothing about the fan-out changes.
            let list_ids: Vec<_> = summaries
                .iter()
                .map(|summary| summary.list_id.clone())
                .collect();
